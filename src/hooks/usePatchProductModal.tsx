...
export default function usePatchProductModal(product?: Product) {
  ...
  const handleOnSubmit = async (values: Product) => {
    ...
    for (const newInfoKey of newInfosKeys)
      if ((product as any)[newInfoKey] !== (newInfo as any)[newInfoKey])
        valuesCopy[newInfoKey] = (newInfo as any)[newInfoKey];
    valuesCopy.id = product!.id;

    const deleteTags = productTags.filter((tag) => !newTags.includes(tag));
    if (deleteTags.length > 0) valuesCopy.deleteTags = deleteTags;
    const addTags = newTags.filter((tag) => !productTags.includes(tag));
    if (addTags.length > 0) valuesCopy.addTags = addTags;

    try {
      await http.patch("/product", valuesCopy);
